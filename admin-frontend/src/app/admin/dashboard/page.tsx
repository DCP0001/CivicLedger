...
function WhitelistVoterButton({ electionId, voter, actionLoading, onWhitelist }: WhitelistVoterButtonProps) {
  ...
  const checkStatus = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/blockchain/is-registered?electionId=${electionId}&walletAddress=${voter.walletAddress}`);
      const data = await res.json();
      setIsWhitelisted(data.isRegistered);
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    checkStatus();
  }, [electionId, voter.walletAddress]);
...
export default function AdminDashboard() {
  ...
  const handleCreateElection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!bcResponse.ok) {
        throw new Error(bcData.error || 'Failed to submit election transaction to blockchain');
      }

      // 2. Write to Supabase or Backend (redundant state synchronization)
      if (isSupabaseConfigured()) {
        const { error: sbErr } = await supabase!
          .from('elections')
          .insert([{
        ...
        if (sbErr && !isTableMissing(sbErr)) {
          console.error('Supabase sync failed:', sbErr.message);
        }
      }

      showMsg(`Election "${elTitle}" created and deployed successfully on custom blockchain!`, 'success');
      setElId('');
      setElTitle('');
      setElDesc('');
      setElStart('');
  ...
  const handleAddCandidate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const defaultImageUrl = 'https://images.unsplash.com/photo-1540910419892-4a36d2c3266c?auto=format&fit=crop&w=120&q=80';

      // Sign candidate transaction
      const type = 'ADD_CANDIDATE';
      const sender = address.toLowerCase();
      const payload = {
        electionId,
        id: candidateId,
      ...
      const bcData = await bcResponse.json();
      if (!bcResponse.ok) {
        throw new Error(bcData.error || 'Failed to submit candidate transaction to blockchain');
      }

      // Write to Supabase or Backend (redundant sync)
      if (isSupabaseConfigured()) {
        const { error: sbErr } = await supabase!
          .from('candidates')
          .insert([{
      ...
      showMsg(`Candidate "${candName}" added successfully to election!`, 'success');
      setCandId('');
      setCandName('');
      setCandParty('');
      setCandDesc('');
      ...
      showMsg(err.message || 'Failed to add candidate', 'error');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const whitelistVoterOnChain = async (electionId: number, wallet: string) => {
    ...
    try {
      ...
      const bcData = await bcResponse.json();
      if (!bcResponse.ok) {
        throw new Error(bcData.error || 'Failed to submit whitelist transaction to blockchain');
      }

      showMsg(`Voter wallet address whitelisted on custom blockchain!`, 'success');
    } catch (err: any) {
  ...
  const startElectionVoting = async (electionId: number) => {
    ...
    try {
      const type = 'START_ELECTION';
      const sender = address.toLowerCase();
      const payload = { electionId };
      ...
      const bcData = await bcResponse.json();
      if (!bcResponse.ok) {
        throw new Error(bcData.error || 'Failed to submit start election transaction');
      }

      // Sync to Supabase or Backend (redundant sync)
      if (isSupabaseConfigured()) {
        const { error: sbErr } = await supabase!
          .from('elections')
      ...
      showMsg('Election opened and active for public voting on custom blockchain!', 'success');
      fetchData();
    } catch (err: any) {
  ...
  const endElectionVoting = async (electionId: number) => {
    ...
    try {
      ...
      const bcData = await bcResponse.json();
      if (!bcResponse.ok) {
        throw new Error(bcData.error || 'Failed to submit end election transaction');
      }
      ...
      showMsg('Election completed! Results locked and computed on custom blockchain.', 'success');
      fetchData();
    } catch (err: any) {
