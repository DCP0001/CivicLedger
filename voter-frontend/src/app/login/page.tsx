...
import { useWeb3Account, useWeb3SignMessage, useWeb3Disconnect } from '@/hooks/useWeb3';
import { useAuthStore } from '@/store/authStore';
import { supabase, isSupabaseConfigured, isTableMissing } from '@/config/supabase';
import { recoverMessageAddress } from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import {
  Wallet, LogIn, AlertCircle, Loader2, ShieldCheck, Activity, ArrowRight
} from 'lucide-react';
...
export default function VoterLogin() {
  ...
  const { setVoterSession, voterToken, setLocalWallet } = useAuthStore();
      ...
      <div className="w-full max-w-md space-y-6 fade-slide-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Connect your browser key credentials to access your voter dashboard and check your verification status.
          </p>
        </div>
            ...
            <div className="space-y-5">
              <div className="text-center space-y-1.5">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Use the same wallet address key you registered with.
                </p>
              </div>
                ...
                onSubmit={(e) => {
                  ...
                  if (key.startsWith('0x') && key.length === 66) {
                    try {
                      ...
                      setLocalWallet(key, acc.address);
                      setLocalKeyInput('');
                      setError(null);
                    } catch (err) {
                    ...
                    setError('Private key must be a 64 or 66 character hex string');
                  }
                }}
                className="space-y-3"
              >
                <label htmlFor="local-voter-key" className="text-xs font-semibold text-slate-500 block">
                  Enter Local Wallet Private Key
                </label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="bg-white dark:bg-[#0c101d] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-750 flex-1 rounded-xl px-3.5 py-3 text-sm font-bold outline-none text-center font-mono focus:border-blue-500/50 transition-colors"
                  />
                  <button
                    type="submit"
                    className="bg-blue-650 hover:bg-blue-750 text-white text-xs font-bold px-4 rounded-xl transition-all shadow-sm active:scale-95 shrink-0"
                  >
                    Connect
                  </button>
                </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 rounded-xl p-4 space-y-2">
                ...
                {[
                  'Connect your registered local private key',
                  'Sign a one-time cryptographic security message (no gas)',
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/40 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-mono text-xs text-emerald-800 dark:text-emerald-350 truncate">{address}</p>
                </div>
              </div>
              ...
              <div className="text-center space-y-1.5">
                ...
                <p className="text-[11px] text-slate-450 leading-relaxed max-w-xs mx-auto">
                  This signature proves you own this private key address. It does not cost any gas fee.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full text-center text-[11px] text-slate-400 hover:text-slate-650 dark:hover:text-slate-300 transition-colors py-1"
              >
