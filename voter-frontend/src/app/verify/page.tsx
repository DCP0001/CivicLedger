...
import { Search, ShieldCheck, Calendar, FileText, Wallet, AlertCircle, Loader2, Database, Layers, Key } from 'lucide-react';
...
function VerifyContent() {
  ...
  const runVerification = async (searchHash: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Transaction not found in blockchain ledger');
      }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-10 py-6">
      <div className="text-center space-y-3">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-405 max-w-lg mx-auto leading-relaxed">
      ...
      <form onSubmit={handleVerify} className="saas-card p-3 flex gap-3 shadow-md">
        <div className="relative flex-grow">
          ...
          <input
            ...
            className="w-full input-premium focus:border-blue-500/50 outline-none rounded-xl py-3.5 pl-12 pr-4 text-xs font-mono text-slate-800 dark:text-slate-205 placeholder-slate-400 transition-all"
          />
        </div>
        <button
          type="submit"
      ...
      {record && (
        <div className="saas-card p-8 space-y-8 border-emerald-200 dark:border-emerald-900/50 relative overflow-hidden group">
          ...
          <div className="space-y-4 text-xs">
            ...
            {record.blockIndex !== undefined && record.blockIndex !== null && (
              <div className="grid grid-cols-3 gap-4 items-center pt-3.5 border-t border-slate-200 dark:border-slate-800">
                <span className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5"><Layers className="h-4 w-4 text-slate-400" /> Block Index:</span>
            ...
            {record.block && (
              <div className="grid grid-cols-3 gap-4 items-center">
                <span className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5">Block Hash:</span>
                <span className="col-span-2 font-mono text-slate-550 dark:text-slate-400 break-all select-all text-[11px]">
                  {record.block.hash}
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-4 items-center pt-3.5 border-t border-slate-200 dark:border-slate-800">
              <span className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5">Voter signature:</span>
              <span className="col-span-2 font-mono text-blue-600 dark:text-blue-450 break-all select-all font-semibold">
                {record.signature || record.transactionHash}
              </span>
            </div>

            {record.block && (
              <div className="grid grid-cols-3 gap-4 items-center pt-3.5 border-t border-slate-200 dark:border-slate-800">
                <span className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[9px] flex items-center gap-1.5"><Key className="h-4 w-4 text-slate-400" /> PoA Authority Sign:</span>
                <span className="col-span-2 font-mono text-emerald-600 dark:text-emerald-450 break-all select-all text-[10px]">
          ...
          <div className="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4">
            <h4 className="text-[10px] font-bold text-slate-400 dark:text-slate-505 uppercase tracking-wider">Blockchain Audit Trail Pipeline</h4>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 relative">
              ...
              <div className="flex items-center gap-3 bg-slate-55 dark:bg-[#0c101d] border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl flex-1 shadow-inner">
              ...
              <div className="flex items-center gap-3 bg-slate-55 dark:bg-[#0c101d] border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl flex-1 shadow-inner">
                ...
                <div>
                  ...
                  <span className="text-[9px] text-slate-450 dark:text-slate-505">
                    {record.blockIndex !== undefined && record.blockIndex !== null ? `Block #${record.blockIndex} Minted & Signed` : 'Ballot verified in block'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-blue-50/50 dark:bg-blue-950/15 border border-blue-100 dark:border-blue-900/40 p-5 rounded-2xl space-y-2">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-405 leading-relaxed">
              This receipt exists permanently on the custom Proof-of-Authority (PoA) blockchain ledger. It has been sealed and signed by the designated election commission authority key.
            </p>
          </div>
        </div>
      )}
