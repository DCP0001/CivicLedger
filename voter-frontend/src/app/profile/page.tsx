    ...
    const fetchStatus = async () => {
      try {
        const [regRes, voteRes] = await Promise.all([
          fetch(`http://localhost:5000/api/blockchain/is-registered?electionId=${election.id}&walletAddress=${walletAddress}`),
          fetch(`http://localhost:5000/api/blockchain/has-voted?electionId=${election.id}&walletAddress=${walletAddress}`),
        ]);
        const regData = await regRes.json();
        const voteData = await voteRes.json();
        setIsWhitelisted(regData.isRegistered);
...
export default function VoterProfile() {
  ...
  const fetchProfileAndElections = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      // Sync verification status into global store only if it changed
      if (userProfile && updateVoterVerificationStatus) {
        if (
          !voterUser ||
          voterUser.verificationStatus !== userProfile.verificationStatus ||
          voterUser.verificationNotes !== userProfile.verificationNotes
        ) {
  ...
  }, [connectedAddress, voterUser?.walletAddress, updateVoterVerificationStatus]);
