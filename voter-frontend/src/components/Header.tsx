...
import { LogOut, UserCheck, Activity, Database, Clock, XCircle, Sun, Moon } from 'lucide-react';
...
export default function Header() {
  ...
  const handleVoterAuth = async () => {
    ...
    try {
      ...
      console.error(err);
      setAuthError(err.message || 'Signature verification failed');
      disconnect();
    } finally {
      setLoading(false);
    }
      ...
      <div className="container mx-auto px-6 py-3.5 flex justify-between items-center gap-4">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-slate-450 dark:text-slate-500 uppercase tracking-widest font-black -mt-0.5">SaaS E-Voting</span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/80 text-slate-500 dark:text-slate-400 transition-colors shadow-xs"
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-[#0c101d] border border-slate-200/60 dark:border-slate-800 px-3.5 py-1.5 rounded-xl text-slate-700 dark:text-slate-300 font-mono text-xs">
                ...
                <span className="text-[9px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wide bg-blue-50 dark:bg-blue-950/20 px-1.5 py-0.5 rounded">
                  {walletMode === 'local' ? 'Local Dev' : 'Localhost'}
                </span>
            ...
            <div className="flex items-center gap-2">
                ...
                onSubmit={(e) => {
                  ...
                  if (key.startsWith('0x') && key.length === 66) {
                    try {
                      ...
                      setAuthError('Invalid private key format');
                    }
                  } else {
                    setAuthError('Private key must be a 64 or 66 character hex string');
                  }
                }}
              ...
              >
                <input
                  type="password"
                  value={localKeyInput}
                  placeholder="Enter Private Key (0x...) to Connect"
                  onChange={(e) => setLocalKeyInput(e.target.value)}
                  className="bg-white dark:bg-[#0c101d] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-1.5 text-xs outline-none font-mono w-64 focus:border-blue-500/50 transition-colors text-center"
                />
                <button
                  type="submit"
                  className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3.5 py-1.5 rounded-xl transition-all shadow-sm active:scale-95 shrink-0"
                >
                  Connect
                </button>
              </form>
