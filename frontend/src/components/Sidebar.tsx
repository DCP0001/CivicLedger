...
import { 
  LayoutDashboard, Vote, BarChart3, User, Settings, 
  ChevronLeft, ChevronRight, Shield, Plus, Users, Activity, FileText
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const { adminToken, voterToken, voterUser, adminUser } = useAuthStore();
  ...
  const adminItems = [
    ...
    { label: 'Analytics & Logs', icon: Activity, path: '/verify' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 py-6 px-4 space-y-7 overflow-y-auto">
        {isVoter && (
          <div>
            <span className={`text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-3 px-3 transition-opacity ${
            ...
            <nav className="space-y-1">
              {voterItems.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`h-4.5 w-4.5 shrink-0 ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-350'}`} />
      ...
      <div className="p-4 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-end">
        <button
          ...
          className="p-2 rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"
        >
