...
import { Vote, Award, ShieldCheck, HelpCircle, Loader2, CheckCircle2, ChevronLeft } from 'lucide-react';
...
export default function CastVote() {
  ...
  // 1. Read on-chain registration state
  const { data: isRegisteredOnChain } = useReadContract({
    address: SECURE_VOTE_ADDRESS,
    abi: SECURE_VOTE_ABI,
    functionName: 'isRegistered',
    args: [BigInt(electionId), address as `0x${string}`],
    query: { enabled: !!address },
  });

  // 2. Read on-chain hasVoted state
  const { data: hasVotedOnChain, refetch: refetchHasVoted } = useReadContract({
    address: SECURE_VOTE_ADDRESS,
    abi: SECURE_VOTE_ABI,
  ...
  // Sync vote to database after blockchain confirmation
  useEffect(() => {
    if (isConfirmed && hash) {
      syncVoteToBackend(hash);
    }
  }, [isConfirmed, hash]);

  const syncVoteToBackend = async (txHash: string) => {
    try {
      if (isSupabaseConfigured() && voterUser) {
        ...
        if (!error) {
          ...
          setErrorMsg(`On-chain transaction succeeded, but Supabase sync failed: ${error.message}`);
          return;
        }
      }
      ...
      if (response.ok) {
        ...
        setErrorMsg(`On-chain transaction succeeded, but database sync failed: ${errorData.error}`);
      }
    } catch (err) {
      console.error(err);
      setErrorMsg('Failed to sync vote transaction reference with off-chain logs.');
    }
  };
  ...
  const handleCastVote = async () => {
    ...
    try {
      // Broadcast the vote transaction via MetaMask
      ...
      console.error(err);
      setErrorMsg(err.shortMessage || err.message || 'Transaction rejected by wallet');
    }
  };
      ...
      <div className="bg-white dark:bg-[#0f1524] border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          {[
            ...
            { step: 4, label: 'Sign Transaction', done: isConfirming || isConfirmed },
          ...
          <div className="space-y-1 flex-1">
            ...
            {selectedCandidate !== null ? (
              <p className="text-xs text-slate-600 dark:text-slate-400">
                You are casting a vote for <span className="text-blue-600 dark:text-blue-400 font-bold">{candidates.find(c => c.id === selectedCandidate)?.name}</span>. This action registers an irreversible transaction on the Ethereum blockchain.
              </p>
            ) : (
      ...
      {hasVotedOnChain && (
        <div className="saas-card text-center py-10 max-w-xl mx-auto space-y-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-450 max-w-sm mx-auto leading-relaxed">
            Our smart contract registers that you have cast a ballot in this election. Duplicate voting is restricted on-chain to maintain platform integrity.
          </p>
        </div>
      )}
    </div>
